...
- **Host form is per-scheduler; `host_regex` stays broad (`'[^/]+'`).** ParallelCluster uses `.pcluster` (`set_host: "host=$(hostname | awk '{print $1}').<%= cluster %>.pcluster"`, matching live VNC); **PCS uses the bare short hostname** (`configure_ood_for_pcs.sh:114` — PCS has no `.pcluster` private hosted zone). A `.pcluster`-scoped `host_regex` would reject PCS nodes, so it stays `'[^/]+'` (Gap 3 tightening is infeasible under dual-scheduler support). `set_host` therefore lives in each scheduler's config generator, not a single shared value: ParallelCluster via the app `submit.yml.erb` (installed by `install_ood.sh`); PCS via `configure_ood_for_pcs.sh`, which overwrites `submit.yml.erb` (now `template: dcv` + bare `set_host` + `-t`).
